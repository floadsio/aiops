...
- When a tmux window launches with Gemini selected, aiops also copies that user's `google_accounts.json` / `oauth_creds.json` (and a safe fallback `settings.json`) into the live CLI directory (`~/.gemini`) before running the command so the official CLI picks them up even if it ignores environment overrides.
