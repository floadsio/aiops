...
@api_v1_bp.route("/issues/preview-assisted", methods=["POST"])
@require_api_auth(scopes=["read"])
@audit_api_request
def preview_assisted_issue():
    """Generate a preview of an issue using AI without creating it.

    This is the first step in AI-assisted issue creation. It generates the content
    that will be used to create the issue, allowing the user to review before committing.
    ...
    Returns:
        JSON response with:
            ...
            - description: Formatted issue description
            - labels: List of appropriate labels
            - branch_prefix: 'feature' or 'fix'
            - preview_token: Token to use in confirm endpoint (session-based)
    """
    ...
    try:
        ...
        current_app.logger.exception("Unexpected error in preview_assisted_issue")
        return jsonify({
...
def create_assisted_issue():
    ...
    This is the second step in AI-assisted issue creation. It uses the preview
    generated by /preview-assisted to create the actual issue.

    Request body:
        preview_token (str): Token from preview_assisted endpoint
    ...
    if preview_key not in flask_session:
        return jsonify({"error": "Preview token expired or invalid"}), 400
    ...
    try:
        # Step 1: Create the issue and persist it locally
        try:
