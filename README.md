...
- Set `GEMINI_APPROVAL_MODE` (defaults to `yolo`) if you want aiops to automatically append `--approval-mode <value>` to the Gemini CLI command; if you provide your own `GEMINI_COMMAND` that already contains the flag, aiops leaves it untouched.
